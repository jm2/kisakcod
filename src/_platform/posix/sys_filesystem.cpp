...
// itself. Returned once directoryFd is empty. depthRemaining bounds the
// recursion so a pathological tree cannot overflow the call stack.
bool RemoveTreeAt(const int directoryFd, const std::size_t depthRemaining)
{
    if (depthRemaining == 0)
    {
    ...
    if (ownedFd < 0)
    {
        SetRemoveTreeStage("open-directory");
        return false;
    }
    ...
    while (const dirent *const entry = readdir(directory))
    {
        ...
        else
        {
            ...
            // erase them silently.
            SetRemoveTreeStage("unspecial-handle");
            failed = true;
            break;
        }
        errno = 0;
    ...
    if (errno != 0)
        SetRemoveTreeStage("enumerate"), failed = true;
    if (closedir(directory) != 0)
        SetRemoveTreeStage("close-directory"), failed = true;
    if (failed)
        return false;
    ...
    for (const std::string &file : files)
    {
        ...
        {
            SetRemoveTreeStage("unlink-file");
            return false;
        }
    }
    ...
    for (const std::string &subdirectory : subdirectories)
    {
        ...
        if (!RemoveTreeAt(childFd, depthRemaining - 1))
        {
            close(childFd);
            return false;
        }
    ...
    for (const std::string &symlink : symlinks)
    {
        ...
        {
            SetRemoveTreeStage("unlink-symlink");
            return false;
        }
    }
    return true;
...
bool KISAK_CDECL Sys_FileSystemRemoveTree(const char *const utf8Path)
{
    ...
    if (parentFd < 0)
    {
        SetRemoveTreeStage("open-root-parent");
        return false;
    }
    ...
    for (std::size_t index = 0; index + 1 < components.size(); ++index)
    {
        ...
        if (nextFd < 0 || !parentClosed)
        {
            if (nextFd >= 0)
                close(nextFd);
            SetRemoveTreeStage("open-parent");
            return false;
        }
        parentFd = nextFd;
    }
    ...
    bool removed = RemoveTreeAt(leafFd, kSysFileSystemMaximumRecursionDepth);
    if (removed
