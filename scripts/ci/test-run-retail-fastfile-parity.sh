...
# The REAL harness derives its own identity, so a mint from this Linux tree
# records leg=linux-amd64. Minting "as windows-x86" from this tree is the
# laundered reference the driver must refuse — covered below.
...
expect_said "minted reference records the derived platform" "platform=linux-amd64" "$LINUX_ENV_REF"
expect_said "minted reference records the validated leg" "leg=linux-amd64" "$LINUX_ENV_REF"
...
# the foreign-field checks refuse it.
...
# widened walk itself lands (protocol-boundary fake; see header).
...
# These probes run the built binary directly: a foreign --leg must be
# refused at capture time, and the accepted leg must appear alongside the
# derived platform in the protocol. The expected identity is read from the
# harness itself so the test holds on any leg platform, not just linux-amd64.
