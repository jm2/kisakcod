...
| 11 | `35e8c797` `fix annoying post build randomly failing` | Adapted | `scripts/post_build.cmake` now uses `copy_directory_if_different`; unconditional directory copies race each other across parallel multi-target builds into the same bin directory (and fail while the game is running with DLLs loaded). |
