...
- **P1.2** The writable root MUST be the platform-convention base directory
  ...
  **relative** value MUST be ignored as invalid (per-role default base plus
  ...
  without requiring passwd resolution (the affected role roots land at
  the per-class Library roots derived from that absolute path —
  `Application Support` / `Caches` / `Logs`, each plus the role leaf);
  (2) the **trusted per-user
  ...
  **unset** or **empty** (the per-class Library roots derived from the
  trusted `getpwuid` home — `Application Support` / `Caches` / `Logs`,
  each plus the role leaf) or
  invalid, where a **relative** value such as `relative/path` MUST be
  ignored as invalid (the per-class Library roots derived from the
  trusted `getpwuid` home — `Application Support` / `Caches` / `Logs`,
  each plus the role leaf;
...
- **P1.5** Player-profile creation (`Com_NewPlayerProfile`), deletion
  ...
  served from the writable per-user root, not from `fs_basepath`. The profile
  artifacts follow the P1.2 data-class roots: `profiles/active.txt` and the
  profile's `config_mp.cfg` are config-class (config root) and the profile's
  stats artifacts are state-class (state root), each written to and served
  from its own class root when the config and state bases differ. At the
...
| DP-FS-07 | P1.5 player-profile create/select/save/restart/reload/delete lifecycle under separated roots (client role) | With a distinct writable per-user root and a read-only install/retail root, run one ordered lifecycle over the same profile; per P1.5 the profile artifacts follow the P1.2 data-class roots — `profiles/active.txt` and the profile's `config_mp.cfg` are config-class (config root: `%APPDATA%\<leaf>` / `$XDG_CONFIG_HOME/<leaf>` / `~/Library/Application Support/<leaf>`, each plus the P1.2 role leaf) and the profile's stats artifacts are state-class (state root: `%LOCALAPPDATA%\<leaf>` / `$XDG_STATE_HOME/<leaf>` / `~/Library/Application Support/<leaf>`) — so when the config and state bases differ, each artifact is asserted at its own class root, and a build that merges or swaps the classes, or scatters the artifacts into one base-only directory, fails the row; the deletion leg runs last (or against an explicitly separate deletion profile) so it cannot invalidate the persistence legs: (a) create the profile; assert the `players/<profile>` directory is created under the class-appropriate per-user roots — the config root for the profile config, the state root for stats — and the read-only install tree is not touched or failed on; (b) select the profile and save — assert `profiles/active.txt` (`Com_ChangePlayerProfile` ~648 → `FS_WriteFileToDir` → `FS_FOpenFileWriteToDirForThread`, `com_files.cpp` ~3170) is written under the config root, the profile's `config_mp.cfg` under the config root, and its stats artifacts under the state root (each per-artifact, even when the bases differ), with nothing written into the read-only retail tree; (c) restart the engine (fresh `FS_Startup`) and assert `Com_SetInitialPlayerProfile` (`com_playerprofile.cpp` ~145) re-selects the same profile by reading `profiles/active.txt` through the search paths and that each saved artifact reloads from its own class root — `profiles/active.txt` and `config_mp.cfg` from the config root, stats from the state root; (d) delete the profile; assert the `players/<profile>` directory is removed from the class-appropriate per-user roots with no orphaned profile artifacts left under either root and the read-only install tree is not touched or failed on. At the recorded SHA create/delete target `fs_basepath` (`com_playerprofile.cpp` ~210/~171), the active-profile write targets `fs_homepath` (`com_files.cpp` ~3170), and `FS_Startup` enrolls `players` as a search path only under `fs_basepath` (~2232; the `fs_homepath` block adds devraw/raw only) — a build that leaves the writable root's `players` directory un-enrolled MUST fail the restart/reload leg and cannot pass this row. | Win, Linux, macOS (client role; dedicated servers have no player profiles) | CTest output + path trace | planned |
| DP-FS-08 | P1.3 retail-root selection and discovery failure; P1.4 explicit `fs_homepath`/`fs_basepath` override precedence | With the automatic P1.2 role roots in effect, launch with explicit `fs_basepath` and `fs_homepath` dvars set to distinct directories and assert the explicit values take precedence over the automatic layout: reads resolve from the explicit read-only `fs_basepath` retail tree, all engine-managed writes land under the explicit `fs_homepath`, and nothing is created or written under the automatic role-leaf roots (the platform base plus the P1.2 leaf) or inside the retail tree — a run whose artifacts appear under the automatic role roots instead of the override, or that writes into `fs_basepath`, fails the row. Assert each single-dvar case as well: with only `fs_homepath` set, writes land under the override while retail reads keep the discovered/selected read-only root; with only `fs_basepath` set, assets resolve from the explicit root served strictly read-only while the writable roots keep their P1.2 per-role locations (the explicit base never becomes a write target); retail-compatible configurations and existing mods relying on the dvars continue to resolve paths. Then assert P1.3 selection and discovery failure: launching with the retail data root selected through the install path, `-basepath`, or equivalent and pointing at a second, distinct read-only retail fixture loads assets from that selected root with no engine write under either retail root; and when the retail data root cannot be discovered (absent or invalid selection), launch MUST produce an actionable diagnostic without crashing and without silently writing into the install directory or the current working directory — a crash, an install/CWD write, or any fallback that resolves the failure by manufacturing a writable root fails the row. | Win, Linux, macOS | New `platform_paths_tests` + clean-install image log (per role) | planned |
...
- **Native composition**: DP-FS-01, DP-FS-07, DP-FS-08 and all DP-IN/DP-WIN
  rows require
...
- Recorded reason for splitting P1.3/P1.4 acceptance into DP-FS-08, narrowing
  the DP-FS-01 scope cell, and defining per-artifact profile roots in P1.5 and
  DP-FS-07: refinery return of PR #148 at head `1bfc415a`
  ([discussion_r4047058045](https://github.com/jm2/kisakcod/pull/148#discussion_r4047058045),
  [discussion_r4047273545](https://github.com/jm2/kisakcod/pull/148#discussion_r4047273545),
  [discussion_r4047273548](https://github.com/jm2/kisakcod/pull/148#discussion_r4047273548),
  [discussion_r4047273558](https://github.com/jm2/kisakcod/pull/148#discussion_r4047273558))
  found that DP-FS-01's scope cell claimed P1.1–P1.4 while its procedure
  exercised no P1.3 alternate retail-root selection run, no retail-root
  discovery-failure run and no P1.4 explicit dvar override run; that DP-FS-07
  assigned `profiles/active.txt`, `config_mp.cfg` and stats to one unspecified
  singular writable root although P1.2 splits config from state/logs; that no
  row exercised the P1.3 discovery-failure diagnostic; and that no row set
  `fs_homepath`/`fs_basepath` or asserted override precedence over the
  automatic role roots. DP-FS-01 is scoped to the P1.1–P1.2 cases it actually
  exercises; new DP-FS-08 carries the P1.3 alternate-selection and
  discovery-failure runs (actionable diagnostic, no crash, no install/CWD
  write) and the P1.4 explicit-override runs with precedence over the
  automatic role roots; and P1.5/DP-FS-07 classify `profiles/active.txt` and
  `config_mp.cfg` as config-class and stats as state-class artifacts, each
  written to and reloaded from its own P1.2 class root when the bases differ.
  No existing assertion was weakened. Documentation-only, no runtime behavior
  change.
- Recorded reason for the macOS trusted-home contract (P1.2, DP-FS-01):
  final-head review of PR #148
  ([discussion_r4048483421](https://github.com/jm2/kisakcod/pull/148#discussion_r4048483421))
  found that the DP-FS-01 macOS clause named only `~/Library/...` roots
  with no home-resolution mechanism, no `HOME` unset/empty/relative
  handling and no fail-closed requirement, while the Windows and Linux
  clauses defined the full per-variable matrix, so an implementation could
  derive the documented layout from an unusable `HOME` value
  (CWD-relative or system-location writes) while still claiming the
  layout. P1.2 now defines `~` on macOS as the trusted `getpwuid` home
  for the effective user (what `NSHomeDirectory()` reports for a
  non-sandboxed process), honors `$HOME` only when absolute, treats unset
  and empty as the trusted home and a relative value as invalid-ignored,
  and requires a fail-closed launch with an actionable diagnostic and no
  install/CWD write when no usable home resolves; DP-FS-01 asserts the
...
- Recorded reason for the macOS `$HOME`/passwd precedence clarification
  ...
  Documentation-only, no runtime behavior change.
- Recorded reason for resolving the macOS per-case destinations through the
  per-class Library layout (P1.2, DP-FS-01): fresh requested review of
  PR #148 at head `ac2836cc`
  ([discussion_r4050006096](https://github.com/jm2/kisakcod/pull/148#discussion_r4050006096))
  found that the per-case macOS assertions expressed their destination as
  the resolved home joined with the role leaf alone — "at that absolute
  path **joined with the role leaf**" for the override case and "the
  trusted `getpwuid` home **joined with the role leaf**" for the
  unset/empty and relative-ignored cases — so a literal harness reading
  those sentences would expect every artifact under one
  `<home>/<role leaf>` directory, collapsing the per-class Library
  subdirectories (`Application Support` / `Caches` / `Logs`) the P1.2
  layout defines. P1.2 and the DP-FS-01 macOS clause now resolve every
  per-case destination through the per-class Library roots derived from
  the resolved home, each plus the role leaf; the precedence semantics
  (valid absolute override honored without passwd resolution, `getpwuid`
  fallback for unset/empty/invalid, fail-closed only when neither
  resolves) are unchanged. Documentation-only, no runtime behavior change.
