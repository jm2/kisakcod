...
- a coherent FX save-snapshot publication boundary (locally complete, PR CI pending) that admits camera/time/visibility
...
2. Complete FX archive runtime closure. Live generation-checked sidecars, full-capacity rollback, exhaustive
   ...
   and the largest other helper at 2,064 bytes. Coherent camera/scalar/visibility snapshot publication is locally complete;
   finish its Windows x86 production/review gate, then start the fixed Disk32 archive schema.
...
membership are locally complete; the next sequence is authoritative Windows x86 review/CI followed by the Disk32 FX
archive schema. A checked
