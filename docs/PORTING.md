...
in run **29446277872** before merge. Wire I/O, both native64 guards, and the writer remain unchanged pending the unified
Disk32 reader.
...
`agent/fx-archive-portable-reader-disk32` sequence adds a portable, non-publishing reader workspace with a lightweight
`BodyState` header, exact `BodyStateDisk32` decode, Ready-only physics enumeration, exact definition-lease validation, and
