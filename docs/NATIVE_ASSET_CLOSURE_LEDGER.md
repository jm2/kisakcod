...
| shader root + `name` | `Load_MaterialVertexShader` (`:3767-3788`); pixel `Load_MaterialPixelShader` (`:3832-3853`) | root `Load_Stream` `:3769-3772` (pixel `:3834-3837`); `prog.vs`/`prog.ps` pre-null `:3773`/`:3838`; name walk `varXString = &…->name; Load_XString(0)` `:3779-3780` (pixel `:3844-3845`); program dispatch `:3786-3787` (pixel `:3851-3852`) | frozen `kMaterialVertexShaderBytes=16` / `kMaterialPixelShaderBytes=16` (`db_disk32.h:22-23`); `name` = XString grammar: 4-byte token, inline `-1` → `AllocLoad_raw_byte` + `Load_XStringCustom` (`:1984-1989`), offset → `DB_ConvertOffsetToCString` block-4 (`:1992-1994`), null left null for caller validation | XString inline/offset (Alloc/Disk32 column) | missing-name `ERR_DROP` `:3774-3777` (pixel `:3839-3842`); completed-empty-name `ERR_DROP` `:3781-3784` (pixel `:3846-3849`) |
| shader root + program | `Load_MaterialVertexShaderProgram` (`:3729-3746`) → `Load_GfxVertexShaderLoadDef` (`:3673-3699`); pixel `Load_MaterialPixelShaderProgram` (`:3748-3765`) → `Load_GfxPixelShaderLoadDef` (`:3701-3727`) | program stream `:3731-3734` (pixel `:3750-3753`); loadDef walk `:3736` (pixel `:3755`) | program payload = `programSize` DWORDs after `DB_ValidateMaterialShaderLoadDef` `:3679-3686` (pixel `:3707-3714`), alloc `:3688-3690` (pixel `:3716-3718`), `Load_DWORDArray` `:3691-3692` (pixel `:3719-3720`), validate `:3693-3698` (pixel `:3721-3726`); frozen `kMaterialShaderProgramBytes=12` / `kMaterialShaderLoadDefBytes=8` (`db_disk32.h:24-25`) | — | validate fail `:3684-3685` (pixel `:3712-3713`); alloc `:3690` (pixel `:3718`); program `:3693-3698` (pixel `:3721-3726`); headless null `:3743-3744` (pixel `:3762-3763`) |
...
| `sunLight` (64 B = `kGfxLightBytes`) | `Load_GfxLight` (def `:4829`) | `:10786-10833` | inline alloc + range `:10796`; slot `GfxLight` `:10800-10802`; `DB_CompleteObject(kGfxLightBytes)` `:10815-10824` | alias conversion carries an explicit **fixed-schema** extent: `DB_ConvertOffsetToAlias(…, GfxLight, kGfxLightBytes)` `:10828-10831` — one of several explicit-extent alias tokens, **not** the only one: MaterialVertexShader/MaterialPixelShader pass fixed `kMaterialVertexShaderBytes`/`kMaterialPixelShaderBytes` (`:3823-3826`, `:3888-3891`), StringTable passes fixed `kStringTableBytes` (`:9684-9687`), MaterialTextureTable passes the dynamic `textureByteCount` (`:4538-4541`), and GfxImage texture aliases pass `mapType` metadata rather than a byte extent (`:3492-3495`) | extent `ERR_DROP` `:10796` |
...
| Weapon | standalone leaf strings after the mid-block aliases: `szAmmoName`/`szClipName`/`szSharedAmmoCapName` (interleaved inside the materials block between `ammoCounterIcon` and `overlayMaterial`), `szAltWeaponName`, then the five trailing strings `szUseHintString`/`dropHintString`/`szScript`/`fireRumble`/`meleeImpactRumble` (immediately before `DB_PopStreamPos` `:9335`) | `Load_XString(0)` ×9 | `:9217-9222`; `:9231-9232`; `:9325-9334` | zone C-string — shared XString grammar: null → skipped; inline `-1` → `AllocLoad_raw_byte` + `Load_XStringCustom`; else block-4 `DB_ConvertOffsetToCString` (`Load_XString` def `:1979-1997`; `kDirectBlock4` = BlockBit 4 `:34`) | — | — |
| Weapon | `accuracyGraphName[0]` and `accuracyGraphName[1]` — pointer-bearing XString leaves distinct from the `accuracyGraphKnots`/`originalAccuracyGraphKnots` direct-pointer walks documented in the next row (`originalAccuracyGraphName` is not loaded by `Load_WeaponDef`) | `Load_XString(0)` ×2 | `:9249-9250` ([0], immediately before the knots[0] walk); `:9287-9288` ([1], after the knots[0] `DB_ValidateWeaponAccuracyGraphKnots` guard `:9285-9286`, before the knots[1] walk) | zone C-string — same shared `Load_XString` grammar (`:1979-1997`) | — | — |
...
| GfxWorld | `sunLight`'s alias conversion carries an explicit extent (`kGfxLightBytes`, `:10828-10831`) — a fixed-schema extent and one of several explicit-extent alias tokens, not the only one (MaterialVertexShader/MaterialPixelShader fixed `kMaterialVertexShaderBytes`/`kMaterialPixelShaderBytes` `:3823-3826`/`:3888-3891`; StringTable fixed `kStringTableBytes` `:9684-9687`; MaterialTextureTable dynamic `textureByteCount` `:4538-4541`; GfxImage texture aliases carry `mapType` metadata instead of byte extents `:3492-3495` — fixed-schema, dynamic and metadata extents stay distinguishable); portal→cell and aabbTree→smodelIndexes are validated direct pointers; sky/outdoor/probe/lightmap images are image aliases; per-surface materials are material aliases; each `dpvs.smodelDrawInsts[].model` carries the full XModel pointer-token grammar (`:9704-9705`, §9.2); `sunLight` is itself pointer-bearing — its `def` member is a LightDef asset-pointer token (`Load_GfxLightDefPtr(0)`, `:4835-4836`) with the standard inline/insert/alias grammar (`:4793-4827`) and, when inline/shared-inline, a nested `attenuation.image` GfxImage alias (`:4788-4789`) | Zone blocks + GPU vertex buffers (`vd`/`vld`, headless null `:10174-10183`); removed via `DB_RemoveGfxWorld` → `DB_MediaUnloadGfxWorld` (headless no-op `db_registry.cpp:72`) | Cell-graph + aabb-tree + span validation after the walk (`:11036-11081`); every per-cell sub-array has its own block-4 extent `ERR_DROP` |
...
- **Rework of review `0a000670` findings** (two P2 inventory/extent-claim
  corrections; citations re-verified against this head before each edit; the
  `31965d30`, `e59c95d7`, `91337ca4`, `b3cd8218`, and `cb1504f7` corrections
  are retained unchanged):
  - §9.5 now enumerates the shader-name pointer walks as explicit leaves.
    Each Material pass shader root streams its fixed 16-byte body *and*
    separately walks `name` through `Load_XString` (`varXString =
    &…->name; Load_XString(0)`, `db_load.cpp:3779-3780`, pixel
    `:3844-3845`) with the standard XString grammar (4-byte token; inline
    `-1` → `AllocLoad_raw_byte` + `Load_XStringCustom` `:1984-1989`;
    offset → `DB_ConvertOffsetToCString` block-4 `:1992-1994`; null left
    null for caller validation) and two per-shader validation `ERR_DROP`s:
    missing name (`:3774-3777`, pixel `:3839-3842`) and completed-empty
    name (`:3781-3784`, pixel `:3846-3849`). The backwards site range
    `:3769-3744` is replaced with the actual per-stage ranges: root
    `:3769-3772`/`:3834-3837`, program `:3729-3746`/`:3748-3765`, loadDef
    `:3673-3699`/`:3701-3727`, dispatch `:3786-3787`/`:3851-3852`.
  - §9.9's `sunLight` row and §10's GfxWorld row no longer claim `GfxLight`
    is the only alias carrying an explicit extent. The actual guarantee:
    the `sunLight` alias conversion passes the fixed `kGfxLightBytes`
    extent (`:10828-10831`) — one of several explicit-extent alias tokens:
    MaterialVertexShader/MaterialPixelShader pass fixed
    `kMaterialVertexShaderBytes`/`kMaterialPixelShaderBytes`
    (`:3823-3826`, `:3888-3891`), StringTable passes fixed
    `kStringTableBytes` (`:9684-9687`), MaterialTextureTable passes the
    dynamic `textureByteCount` computed after the table walk
    (`:4538-4541`), and GfxImage texture aliases pass `mapType` metadata
    rather than a byte extent (`:3492-3495`). Fixed-schema, dynamic and
    metadata extents stay distinguishable in both rows. Documentation only
    — no production code, serialization, or gate change.
- **Rework of review `5ca03e42` finding** (one P2 inventory correction,
  comment `r4043121685`; citations re-verified against this head before each
  edit; the `31965d30`, `e59c95d7`, `91337ca4`, `b3cd8218`, `cb1504f7`, and
  ...
  - §9.1's grouped `trans->u` row is split to the leaves, and the previously
    ...
    (`:2286-2287`, def `:2244-2270`) — a nested walker that follows the
    on-disk `_1` frame pointer: `smallTrans` allocates via
    `AllocLoad_raw_byte` (`:2253`) and streams `Load_ByteVecArray` at 3-byte
    stride (`:2236`); otherwise `AllocLoad_FxElemVisStateSample` (4-byte,
    `:2263`) and `Load_UShortVecArray` at 6-byte stride (`:2241`);
    `frameCount` is `DB_CheckedCountSum(size, 1, "animation translation
    frames")` only for nonzero size, else 0 (`:2246-2248`), with explicit
    count-0 streams (`:2255-2258`, `:2265-2268`) and an overflow `ERR_DROP`
    (`:1185`). This corrects the earlier `31965d30` audit note: the quat
    delta frames (`:2133-2154`) and delta-indices walks were enumerated, but
    the trans `frames` nested walker was not — it had been folded into the
    in-place "frames/indices" row. No new walker, implementation, or test is
    created; the row documents the existing production walk. Documentation
    only — no production code, serialization, or gate change.
