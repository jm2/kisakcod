...
- No default allowlist, no default limit, no default rejection rule, no default
  dvar flag change, and no change to `CG_SetClientDvarFromServer`'s general path
  in production.
...
The protection design is delivered as a **separate, default-off optional mode**
so that the commercial-compatible default path is byte-for-byte and
behavior-for-behavior unchanged unless an operator explicitly opts in.
...
| C6 | `v` command with an odd argument count (trailing name with no value) | loop reads `Cmd_Argv(i + 1)` past the end |
...
4. **Fail-safe, not fail-fatal, only where fatal is not retail behavior.**
   Changing a client-fatal path is a robustness improvement, not a compatibility
   change, but it still requires evidence that the original builds are not
   fatal at the same point before it can be treated as compatible.
...
Each protection is a design option for the optional mode. "Default" is the
production behavior today. "Optional mode" describes what the opt-in mode would
do. "Reference evidence required" is the evidence that would have to be captured
before the option could be enabled. "Status" is the acceptance state of the
option, not of the A06 task.
...
| HP3 | C3 | Convert the 4096-dvar pool-cap `Com_Error(ERR_FATAL, ...)` (`src/universal/dvar.cpp:1568-1571`) into a bounded, non-fatal refusal in the optional mode, with callers handling the failure. | Original builds may also be fatal at the cap; a graceful refusal diverges, and a code path that assumes registration always succeeds must be audited. | Reference behavior at the pool cap; caller audit for non-NULL assumptions. | BLOCKED — design recorded, not enabled. |
...
| HP6 | C6 | Define deterministic handling of an odd trailing `v` argument in the optional mode (ignore the trailing name, or reject the command) instead of silently reading an empty value (`src/cgame_mp/cg_servercmds_mp.cpp:663-670`). | The original client may tolerate the odd argument and apply a partial command; changing the outcome can diverge. | Reference behavior for a `v` command with an odd argument count. | BLOCKED — do not enable without evidence. |
...
unavailable. Each blocks acceptance of the protection it names. None of them
justifies relaxing the compatibility contract in
...
| T3 | Replacing the fatal pool-cap with a graceful refusal may diverge from the reference and requires a caller audit. | HP3 | Cannot accept until reference pool-cap behavior and all registration call sites are audited. |
...
| T6 | Odd-argument `v` handling changes a tolerated outcome. | HP6 | Cannot accept until reference behavior for an odd argument count is captured. |

Until these are resolved, the correct production state is the unchanged default
path. This is the "record the unresolved tradeoff and block its acceptance"
requirement of the A06 acceptance criteria, applied per protection.

## 7. Optional-mode design and activation safety

The protections above, where not blocked outright, are delivered only through a
separate, default-off optional mode:
...
- What are the reference outcomes for `hud_drawHud` `>1`, negative and
  non-numeric, and for an odd-count `v` command?
