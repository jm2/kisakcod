...
- Current merged baseline: PRs #39--#64 completed earlier audited upstream/gameplay reconciliation checkpoints, the
  ...
  pending-copy ledger, curated upstream typed-sort checkpoint, and production-neutral registry coordinator through
  `7f030c03269235b3ad703c13404e0975f798bd18`. PR #51 exact-head run
...
- Pinned `upstream/master` `2164cd1a` remains **18 commits ahead by ancestry**. The durable disposition for every commit
  is recorded in `docs/UPSTREAM_2164CD1A_LEDGER.md`. The reviewed U1 candidate selectively adapts the surviving
  `r_dpvs` brush-definition, aim-assist, native-width HUD, signed-angle, and command-tail fixes as `faf3917d`,
  `a401775a`, `fc265d13`, and `35960465`; exact U1 evidence is full GCC **152/152** and focused **7/7**. Individual U1
  batches also pass strict GCC/Clang and genuine i386/AArch64 compilation, HUD passes ASan+UBSan, and an independent
  adversarial audit found no actionable defect. `b40fd7bb` combines the separately validated U2 Miles dry-level cleanup;
  the complete GCC Debug suite then passes **153/153** and the combined focus passes **8/8**. U2 independently passes **146/146**, its ten-mutation source
  contract, strict i386 GCC MP/SP and Clang MP compile-link, and a symbol audit proving the wet ABI is unchanged and no
  scoreboard dependency remains. PR #65 initial head `f8bf4e32` received a clean Codex review; Gemini and portable
  MSVC jobs in run **29703553789** exposed the same implicit double-to-float angle conversions plus a real null-name
  command-comparator gap. Review-fix commit `db3ced51` keeps the original staged double arithmetic with explicit float
  conversions, selects `std::floor(float)`, rejects null lookup names, skips null registry names, and adds four
  mutation/source guards plus runtime null cases. Full GCC Debug remains **153/153**, focused review tests pass **3/3**,
  and strict GCC/Clang plus genuine i386/AArch64 compile-link pass for both touched contracts. Replacement exact-head CI
  and review are pending. Dynent save/load,
  raw SP/save/screenshot and gameplay changes, matrix rewrites, and other unbuilt SP work remain explicitly deferred or
  rejected; icon work remains a provenance/optional-packaging item. After the U1/U2 content PR merges, reconcile only
  exact pinned `2164cd1a` with a separately reviewed, tree-identical `-s ours` ancestry checkpoint using merge-commit
  semantics—never squash or rebase it.
...
- Progress estimate: approximately **77% complete by merged engineering effort**. PR #62 merged the production-neutral
  pending-copy ledger, PR #63 merged the curated upstream typed-sort checkpoint, and PR #64 merged the production-neutral
  registry coordinator. The combined U1/U2 upstream content candidate is not counted until it merges; its merge plus the
  exact ancestry checkpoint should move the rounded estimate to approximately **78%**. Windows x86 is about
  **93%**, shared
...
1. Review and merge the combined U1/U2 brush, angle, aim, command, HUD, and Miles dry-level content PR. Keep the
   commit-by-commit dispositions in
   `docs/UPSTREAM_2164CD1A_LEDGER.md` exact; do not import deferred raw SP/save/screenshot, dynent, matrix, gameplay, or
   icon hunks as collateral changes.
2. Re-fetch upstream, then create the exact pinned `2164cd1a` tree-preserving `-s ours` ancestry checkpoint in a separate
   PR. Verify both parent identities and tree equality, and merge it with merge-commit semantics only. Do not squash or
   rebase this checkpoint. Audit newer upstream commits separately if the remote has advanced.
3. Compose the already reviewed durable allocation, stream-generation, pending-copy, and native-storage receipts
   passively into the existing 33-entry runtime table, with the process-wide stream binding and pending ledger owned once
   at table scope. This is layout/lifecycle composition only: mint no generation, claim no PMem scope, and enroll none of
   the seven legacy sites.
4. Complete the checked PMem/global/narrow-resource prerequisites before production enrollment: repair the native64-
   invalid `PMem_FreeIndex`/`PMem_EndAllocInPrim` path and `$init` lifecycle, authenticate the table-wide singleton
   resources, and keep each per-entry receipt capsule's mutable authority narrow and non-duplicating.
5. Add exact-key, production-neutral adapters that reconstruct and authenticate the passive receipt composition and bind
...
6. Only after those gates, perform one atomic seven-site loader cutover: claim exact generations, stage/transfer
