...
- Active registry-coordinator foundation: the rebased branch remains production-neutral and leaves all seven legacy
  sites frozen. Preflight found that the initial per-operation `db_hashCritSect` acquisition would self-deadlock at the
  hash-held production sites, public low-level batch functions could bypass coordinator authority, fallible finish and
  abandonment could clear or strand the global boundary, a corrupt borrowed pointer could be dereferenced before mirror
  ...
  IDs through one callback-free bounded bulk call. The eventual atomic cutover remains five coordinator operations plus
  two exact-key root-journal stages; no partial production enrollment is allowed.
...
- Progress estimate: approximately **76% complete by merged engineering effort**. PR #62 merged the production-neutral
  pending-copy ledger and PR #63 merged the curated upstream typed-sort checkpoint; the active production-neutral
  registry coordinator is not counted until it merges. Windows x86 is about
  **93%**, shared
...
1. Finish and publish the production-neutral registry ownership coordinator that can either borrow the exact active
   controller transaction or own a standalone transaction. Retain one authenticated database-hash scope, keep all five
   fixed report-free operations behind a private exact admission capability, authenticate mirrored lifetime state before
   pointer dereference, poison on unsafe close/abandonment, and apply pre-collected IDs through one callback-free bounded
   bulk call. Prove transaction -> registry -> string -> memory-tree ordering and every failure boundary without changing
   the seven frozen sites.
2. Embed the durable receipts in the existing 33-entry runtime table—never a second generation authority—then perform one
   ...
   terminal adapters, and replace exactly seven total raw sites: five registry-coordinator operations plus two exact-key
   root-journal stages (the transfer/shutdown sweep is included in seven).
...
6. Continue process services and Linux signal-park plus macOS Mach crash freezing behind the now-isolated terminal API;
   add native Win32 headless character-console input without disturbing the windowed edit-control owner.
7. Widen/tokenize the MP pose, breakable-piece, and DynEntity physics ownership families without changing saved bytes;
