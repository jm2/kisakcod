...
- Active registry-coordinator foundation: the final pre-PR candidate remains production-neutral and leaves all seven
  legacy sites frozen. Preflight found that the initial per-operation `db_hashCritSect` acquisition would self-deadlock
  ...
  IDs through one callback-free bounded bulk call. Local seal commit `9f327514` protects the production boundary;
  `74b56b65` adds portable nonblocking hash admission, fixes the real pre-held-reader deadlock, and corrects the real
  i386 coordinator-size test; `90e8fba7` completes portable test wiring. A native GCC Debug build and full CTest pass
  **145/145**, while the focused source/security/macro-off seal selection passes **6/6**. The candidate has zero
  production enrollment. After publication, the immediate implementation batch remains one atomic durable runtime-table/
  loader cutover: five coordinator operations plus two exact-key root-journal stages, with no partial enrollment.
...
1. After the final production-neutral coordinator candidate is reviewed and merged, embed the durable receipts in the
   existing 33-entry runtime table—never a second generation authority—and perform one
