...
- Current global-PMem encapsulation checkpoint: unmerged commit `3b826224` moves `g_mem` and
...
- Progress estimate: approximately **80% complete by merged engineering effort**. PR #62 merged the production-neutral
  ...
  tree-neutral ancestry checkpoint, PR #67 merged passive durable-receipt composition, and PR #68 merged the legacy
  PMem indexing/failure-atomic prerequisite. The ancestry checkpoint records reviewed history without importing code
  and therefore does not inflate the engineering estimate. The current global-state encapsulation checkpoint remains
  outside the merged estimate until its PR lands.
...
1. Publish the implementation commits through `34869793` with this documentation checkpoint, run all nine hosted jobs,
   obtain exact-head review, close every thread, and merge the batch. Preserve whole-type macro gating,
   official-manifest/source/workflow
   containment, actual macro-off object-symbol inspection, the exact legacy ABI, and zero production or checked-authority
   enrollment; no hosted result is claimed before those gates run.
2. Extend that production-neutral runtime boundary with exactly one reacquirable serializer, an authoritative external
   reservation extent, coherent initialization state, report-free allocation results, and bounded by-value diagnostic
   snapshots. Serialize initialization, Begin/End/Free, allocation, getters, and snapshot capture; reserve/commit and
   every assertion, print, OOM report, and `Com_Error` must occur after releasing the serializer.
3. Add the separate unused process-lifetime `$init` controller and authenticate the table-wide singleton resources while
   ...
   legacy zone PMem lifecycle calls so checked and legacy authority cannot overlap.
   Validate concurrent low-prim allocation versus high-prim end, persistent high-prim index zero,
