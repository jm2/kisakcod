...
- Current transactional script-string-journal checkpoint: baseline core `70ef96fc`, hardened/integrated checkpoint
  `d158d5e9`, and exact prepare/finalize implementation `aa295367` provide a production-neutral journal bound to one
  exact `ZoneLoadContextKey`. Fixed caller-owned 8-byte entries preserve one full nonzero `uint32_t` ID per
  ...
  transfer.
- `d158d5e9` makes controller validation O(1); exact implementation `aa295367` performs a complete entry scan only on the
  first seal, prepare, or begin-rollback phase boundary, and validates only the current entry for each transfer/rollback
  step.
...
- Current journal validation at exact implementation `aa295367`: the complete GCC and Clang suites are **82/82** green.
  ...
  portable utility
  jobs to build/run the target and measured Windows x86 Debug/Release to build it explicitly. Candidate CI and hosted
...
1. Finish exact-head validation, hosted review, candidate CI, and merge for the script-string journal. Pin the caller
