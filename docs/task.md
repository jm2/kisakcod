...
- Current merged baseline: PRs #39--#50 completed the audited upstream/gameplay reconciliation, the
  report-free script-string ownership foundation, the constructed zone ownership controller, and script-string
  initialization hardening through `eeca68ba`. Authoritative post-merge run **29627591759** passed all nine jobs:
  Linux amd64/arm64, portable
...
- Active external runtime-table checkpoint: rebased head `636531fe` adds a fixed production-owned table with stable
...
- Active table validation: the complete GCC utility suite is **107/107** green; focused Clang, ASan+UBSan (leak detection
  ...
  prove the legacy loader does not claim or consume the table in this batch. Exact-key terminal reset/Live unload,
...
- Progress estimate: approximately **73% complete by merged engineering effort**. The active durable runtime-table
...
1. Publish the fixed 33-entry production-owned runtime table at `636531fe`. Keep its exact by-value generation authority,
   slot-zero reservation, canonical/poison/phase/serializer matrix, `DB_Init`-before-pool ordering, and source prohibition
   on production claim/consumption green on all five portable hosts plus measured Windows x86.
