...
- Current merged baseline: PRs #39--#66 completed earlier audited upstream/gameplay reconciliation checkpoints, the
  ...
  brush/angle/aim/command/HUD/Miles upstream reconciliation through
  `225759e7d8fd1327210452f3debcd6360465ef2a`. PR #51 exact-head run
...
- Current passive-receipt composition branch: commits `dd8bcd2d` and `82e12e57` compose one stable allocation,
  stream-generation, pending-copy, and native-storage receipt capsule into each of the 33 runtime entries, while owning
  the process-wide active-stream binding and pending-copy ledger exactly once at table scope. The branch performs no
  production enrollment: it mints no generation, claims no PMem scope, admits no pending copy, and changes none of the
...
- Progress estimate: approximately **78% complete by merged engineering effort**. PR #62 merged the production-neutral
  ...
  tree-neutral ancestry checkpoint. The checkpoint records reviewed history without importing code and therefore does
  not inflate the engineering estimate. Passive receipt composition is implemented and fully validated on the current
  branch, but remains outside the merged estimate until its PR lands.
...
1. Publish, exact-head review, and merge the implemented passive receipt composition. Preserve its one-capsule-per-entry
   and one-singleton-per-table topology, complete authority/source seals, and zero production enrollment.
2. Land the narrowly bounded legacy PMem indexing and failure-atomic validation repair. Then add a production-neutral
   global PMem runtime boundary with one serializer, an external authoritative reservation extent, hidden mutable PMem
   state, and a separate unused process-lifetime `$init` controller. Do not cut over `$init` alone: it must remain Ended
   for process life, and its later enrollment must atomically replace both `$init` calls plus all three legacy zone PMem
   lifecycle calls so checked and legacy authority cannot overlap.
3. Authenticate the table-wide singleton resources and keep each per-entry receipt capsule's mutable authority narrow
