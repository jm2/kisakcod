...
- Current merged baseline: PRs #39--#67 completed earlier audited upstream/gameplay reconciliation checkpoints, the
...
- Progress estimate: approximately **79% complete by merged engineering effort**. PR #62 merged the production-neutral
  ...
  tree-neutral ancestry checkpoint, and PR #67 merged passive durable-receipt composition. The ancestry checkpoint
  records reviewed history without importing code and therefore does not inflate the engineering estimate. The current
  legacy-PMem indexing repair remains outside the merged estimate until its PR lands.
...
1. Publish, exact-head review, and merge the implemented legacy PMem indexing and failure-atomic validation repair.
   Preserve the exact x86 ABI, named native64 layouts, typed bounded indexing, fixed-format diagnostics, checked-PMem
   source seal, and zero checked-authority enrollment. The remaining publication gates are all nine hosted jobs and
   exact-head review/thread closure.
2. Add a production-neutral global PMem runtime boundary with one serializer, an external authoritative reservation
   extent, hidden mutable PMem state, and a separate unused process-lifetime `$init` controller. Serialize initialization,
   Begin/End/Free, allocation, and diagnostic snapshots. Do not cut over `$init` alone: it must remain Ended for process
   life, and its later enrollment must atomically replace both `$init` calls plus all three legacy zone PMem lifecycle
   calls so checked and legacy authority cannot overlap.
