...
- [ ] Atomically cut all seven loader sites over to the serialized facade; partial
  ...
  - [ ] Publish and merge the exact-key, by-value pending-copy inspection
    ...
    - [x] Pass final local GCC CTest **171/171**, focused Clang and ASan+UBSan,
      ...
      `ef74688b`.
    - [ ] Publish the PR, address exact-head automated review, pass all hosted
      jobs, and merge the reviewed checkpoint.
...
- Progress estimate: approximately **82% complete by current engineering effort**. PR #62 merged the production-neutral
  ...
  caller; hosted review, CI, and merge remain pending. The ancestry checkpoint records reviewed history without
  importing code and therefore does not inflate the engineering estimate.
...
are clean. No production caller is enrolled; hosted review, CI, and merge are
still pending.
...
- [ ] **Priority 2 — Atomic loader cutover:** claim exact generations, stage
   ...
   - [ ] Publish and merge exact-key pending-copy inspection, eliminating
     ...
     - [ ] Complete exact-head hosted review/CI and merge.
