...
Last reconciled: **July 20, 2026**. A checked parent means that milestone's exit
...
- [ ] Atomically cut all seven loader sites over to the serialized facade; partial
  ...
  - [ ] Publish and merge the exact-key, by-value pending-copy inspection
    ...
    - [ ] Pass the final documentation-only head, merge the reviewed checkpoint,
      and pass authoritative post-merge CI.
...
- Progress estimate: approximately **82% complete by current engineering effort**. PR #62 merged the production-neutral
  ...
  caller. PR #76 code/documentation head `9b0e4c5f` passed all nine jobs in run **29794474261**; final documentation-only
  CI, merge, and authoritative post-merge validation remain pending. The ancestry checkpoint records reviewed history
...
No production caller is enrolled; final documentation-only CI, merge, and
authoritative post-merge validation remain pending.
...
- [ ] **Priority 2 — Atomic loader cutover:** claim exact generations, stage
   ...
   - [ ] Publish and merge exact-key pending-copy inspection, eliminating
     ...
     - [ ] Pass final documentation-only CI, merge PR #76, and pass
