...
- Current transactional script-string-journal checkpoint: baseline core `70ef96fc`, hardened/integrated checkpoint
  `d158d5e9`, prepare/finalize implementation `aa295367`, and exact boundary-hardened candidate `81ded193` provide a
  production-neutral journal bound to one exact `ZoneLoadContextKey`. Fixed caller-owned 8-byte entries preserve one full
  nonzero `uint32_t` ID per
...
- Current journal validation at exact candidate `81ded193`: the complete GCC and Clang suites are **82/82** green.
  ...
  success/pre-`Live` failure tests, and the 65,536-entry path prepares to `CommitReady` before exact rollback. Candidate
  CI and hosted review remain pending.
