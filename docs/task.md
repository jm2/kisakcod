...
- [x] Selectively reconcile the 30 upstream commits through `c6be07a2`
  ...
  - [x] Defer with recorded reasons: the SP vehicle-refactor trilogy, the
    ...
    the OpenAL restructuring epic, the build-number scheme (byte-parity
    tooling dependency), and the post-build copy change (newer than the
    supported CMake 3.16 command surface). Reject the radiant editor series,
    the mechanical warning sweeps, and upstream README/build-policy churn.
  - [x] Record exact upstream ancestry with tree-neutral checkpoint
...
- [x] Audit and record the next 16 upstream commits through `5c27f2af`
  (upstream tip as of September 4, 2026).
  - [x] Record exact per-commit dispositions for `14900b6b` through
    `5c27f2af` in `docs/UPSTREAM_5C27F2AF_LEDGER.md`.
  - [x] Defer the network/save/gameplay-visible enum migrations until their
    numeric values, profile behavior, and serialized contracts have dedicated
    tests; defer `355b68a0` separately because it also changes collision and
    helicopter-water behavior.
  - [x] Record exact upstream ancestry with tree-neutral checkpoint
    `86459c53`; its second parent is exact `5c27f2af`, its first-parent diff
    is empty, and its tree matches first parent `f2408924` at `898ed947`.
...
- [x] Publish the production-neutral serialized runtime facade (PR #73).
  - [x] Implement the private registry bridge, process-lifetime serializer,
    composite-only table/registry forwarding, and fail-closed authority checks.
  - [x] Preserve zero production callers and freeze the seven raw loader sites.
  - [x] Finish the final method-to-adapter, production-macro, and realistic
...
- [x] Atomically cut all seven loader sites over to the serialized facade; partial
  ...
  - [x] Add a private exact-key registry-borrow path authenticated for active
    ...
    - [x] Implement the private facade -> table -> controller -> coordinator
      ...
      authentication while preserving zero production callers.
    - [x] Prevent same-purpose replay with a mirrored, non-wrapping witness;
      preserve coordinator ABI sizes and established mode values.
    - [x] Fail closed on cross-slot/torn markers and two-stage admission failure,
      with saved key/table/callback-tuple post-authentication before later side
      effects.
  ...
  - [x] Publish and merge the exact-key, by-value pending-copy inspection
    ...
    `g_copyInfo` pointers.
    - [x] Implement a pointer-free 0x18 key/count view and ordered record reads
      through the table and serialized facade, with exact generation/count
      reauthentication, stale/ABA rejection, and failure-atomic outputs.
    - [x] Keep pending-copy ledger, receipt, stream, lifecycle, and registry
      authority private; preserve zero production callers and all seven frozen
      loader sites.
    - [x] Pass final local GCC CTest **171/171**, focused Clang and ASan+UBSan,
      genuine i386 and AArch64 compile/link, production/source/security seals,
      `git diff --check`, and a clean independent audit at code checkpoint
      `ef74688b`.
    - [x] Publish PR #76, document and resolve both non-actionable Gemini null
      findings, and pass all nine hosted jobs at exact code/documentation head
      `9b0e4c5f` in run **29794474261**; Codex review was unavailable because its
      repository usage limit was reached.
    - [x] Pass all nine jobs at final documentation-only head `9c11d7a8` in run
      **29795268032**, merge PR #76 as `1e681792`, merge its documentation
      checkpoint as PR #77 at `8e157543`, and pass all nine authoritative jobs
      there in run **29890463505**.
  - [x] Add a report-free readiness-aware PMem storage-isolation classifier and
    apply it to retained legacy admission, rollback, and unload callback
    identities.
    - [x] Authenticate coherent Uninitialized, Initializing, Ready, and Poisoned
      phases under one PMem lock; distinguish invalid ranges, fixed-control or
      retained-extent overlap, and corruption without reporting under the lock.
    - [x] Preserve the legacy Ready-only boolean wrapper; accept null and
      coherent pre-initialization identities, map initialization contention to
      `Busy`, and reject managed/control/poisoned/corrupt identities before
      lower lifecycle mutation.
    - [x] Seal the three call sites and their ordering, prove failure-atomic
      managed-context rejection plus null/stack retry, and pass local GCC
      **171/171**, focused Clang/ASan+UBSan, i386/AArch64 compile-link,
      source/security, diff, and two independent audits at `0a87007e`.
...
- [ ] **M1 — Cross-compiler and ABI hygiene.**
  ...
  - [x] Widen the completed database, DObj, XAnim, EffectsCore, pose, worker-queue,
    and related runtime slices recorded in the evidence log.
  - [ ] Remove the remaining raw-width, compiler-specific, and layout debt needed
    by production 64-bit/POSIX builds.
- [ ] **M2 — Pointer and security hardening.**
  - [x] Land the pointer-width tripwire, Huffman/Disk32 bounds work, remote-input
    hardening, graph validation, and failure-atomic loader/runtime checks recorded
    below.
  - [ ] Add handle-relative no-follow/reparse-safe file operations, production-path
    fuzz fixtures, and the remaining bounded load-object cursor.
  - [ ] Run production map/demo paths cleanly under the required sanitizer gates.
- [ ] **M3 — Platform services.**
  ...
  - [ ] Finish sockets and handle-relative recursive deletion; process launch/control,
    POSIX/Mach crash freezing, and native Win32 headless character-console input have
    landed (see the evidence log).
  - [ ] Populate and link the Linux/macOS production engine source sets. The service
    source sets are fully populated, linked, and runtime-tested on both POSIX presets
...
- [ ] **M7 — Linux amd64 dedicated server.**
- [ ] **M8 — Portable RHI plus native Vulkan/SDL client path.**
- [ ] **M9 — Linux amd64 full client and x86-64 parity baseline.**
- [ ] **M10 — Architecture-neutral ARM64 determinism layer.**
- [ ] **M11 — Windows ARM64 client/server delivery.**
- [ ] **M12 — Linux ARM64 client/server delivery on real hardware.**
- [ ] **M13 — Signed/notarized macOS ARM64 application delivery.**
- [ ] **M14 — Required five-target production CI, packaging, provenance, and
  release gates.**
  - [ ] Align `ci.yml` and `release.yml` with the maintained multi-platform
    patterns in `jm2/CroMagRally` and `jm2/tributary`.
  - [ ] Require production jobs, smoke/parity evidence, target-labeled artifacts,
    immutable tag/SHA validation, least-privilege publishing, a reproducible source
    archive, and aggregate `SHA256SUMS` for every supported platform.
...
- [x] Curate the three source commits through upstream tip `4ad0a2e2`, preserve
  ...
  `1b1ab3d0`. PR #75 final head `7842d83c` passed all nine jobs in run
...
- Merged PR #71 exact-key composite-controller milestone: the merged controller completes the report-free PMem
  ...
  **157/157**; the repair-head Clang and ASan+UBSan affected selection is **38/38** in each tree. Exact head `a65ff336`
  ...
  exact master commit. This completes the production-neutral exact-key adapter queue item. PR #72 exact final head
  `0c1354c05c92f2f9e66ce285d85d71c90a4119eb` passed all nine jobs in run **29768665063**, with clean exact-head
  Codex/Gemini review and zero unresolved threads. It squash-merged the ABI-neutral capacity/demand split and symmetric
  storage-pair preflight as `64dfc8c9e3b930a7ef7760b794e37320471280c3`; authoritative post-merge run
  **29770265354** passed all nine jobs. PR #73 then published the production-neutral serialized runtime
  facade: one process-lifetime nonblocking outer serializer, mirrored global/TLS ownership, a private
...
- Merged serialized-runtime foundation: PR #70 sequence `293a020c` appends `CRITSECT_PHYSICAL_MEMORY` at MP `0x17` and SP
  ...
  absent at that merge; PR #71 subsequently merged the production-neutral adapter/controller layer without enrollment.
...
- Progress estimate: approximately **84% complete by current engineering effort**. PR #62 merged the production-neutral
  ...
  production seal, PR #70 merged serialized runtime/process-controller/passive shared-resource authentication, and PR
  #71 merged the production-neutral exact-key adapter layer, exact terminal PMem evidence, strict table orchestration,
  alias/overlap hardening, and adversarial composition coverage without production enrollment. PR #72 merged the
  ABI-neutral capacity/demand split, PR #73 merged the production-neutral serialized runtime facade, PR #74 merged
  private callback-scoped registry borrowing, and PR #75 reconciled upstream through `4ad0a2e2`. Code checkpoint
...
## Detailed milestone evidence

The canonical completion state is the checkbox dashboard at the top of this
file. This long-form table is retained as historical evidence through PR #72;
later merged and active work is recorded in the dashboard and immediate queue
instead of continually rewriting the deliberately compact evidence snapshot.

| Milestone | Status | Evidence snapshot through PR #72 |
|---|---|---|
...
coverage does not satisfy an engine-delivery box.
...
sizes, treats unrelated-slot contention as `Busy`, poisons torn or contradictory
markers, and suppresses later admission effects after any callback failure.
...
managed-PMem aliases, fail closed on lifecycle/marker corruption, and never
publish partial output. Final local GCC CTest is **171/171**; focused Clang,
ASan+UBSan, genuine i386/AArch64 compile-link, macro-off production seals,
source/security contracts, `git diff --check`, and an independent final audit
are clean. PR #76 exact code/documentation head `9b0e4c5f` passed all nine jobs
in run **29794474261**. Gemini's two null-pointer findings were documented as
non-actionable short-circuit/unevaluated-`sizeof` false positives and resolved;
Codex review was unavailable because its repository usage limit was reached.
...
- [x] **Priority 1 — Facade publication:** PR #73 landed the
   ...
   serializer and private registry-coordinator bridge. It preserves mirrored
   fail-closed ownership, runtime -> transaction -> hash -> script-string ->
   memory-tree ordering, composite-only script adapters, release refusal while
...
- [x] **Previous Priority 0 — Upstream reconciliation through `4ad0a2e2`:**
   ...
   ancestry checkpoint `1b1ab3d0`. PR #75 final head `7842d83c` passed all nine
...
- [ ] **Priority 2 — Atomic loader cutover:** claim exact generations, stage
   ...
   - [x] Publish and merge the private exact-key registry borrow
     ...
     - [x] Bind every borrow to the exact generation, transaction, callback
       purpose, thread, and non-reusable callback-window witness.
     - [x] Preserve ABI/mode compatibility and fail closed across nested callback
   ...
   - [x] Publish and merge exact-key pending-copy inspection, eliminating
     ...
     - [x] Implement the pointer-free key/count snapshot and by-value ordinal
       read through table and facade at `ef74688b`.
     - [x] Pass the complete **171/171** local GCC suite and the affected
       compiler, sanitizer, architecture, production-seal, source/security,
       diff, and independent-audit gates.
     - [x] Pass all nine hosted jobs at `9b0e4c5f` in run **29794474261** and
       resolve both non-actionable Gemini threads; Codex review was unavailable
       because its repository usage limit was reached.
     - [x] Pass all nine jobs at final documentation-only head `9c11d7a8` in
       run **29795268032**, merge PR #76 as `1e681792`, merge PR #77 as
       `8e157543`, and pass all nine authoritative jobs there in run
       **29890463505**.
   - [x] Add the report-free seven-state
     `TryClassifyStorageIsolation` query and enroll it at the retained legacy
     admission, rollback, and unload callback-identity boundaries.
     - [x] Authenticate complete PMem phase coherence and fixed-control/retained
       extent separation in one serialized observation; preserve
       `StorageIsOutsideManagedMemory` as true only for coherent Ready and
       disjoint storage.
     - [x] Accept null or coherent Uninitialized one-byte identity anchors,
       return `Busy` during initialization, and reject managed, protected,
       poisoned, or corrupt contexts before lower mutation.
...
- [ ] **Priority 6 — Remaining platform services:** continue process services,
   Linux signal-park, and macOS Mach crash freezing behind the now-isolated
   terminal API;
...
- [ ] **Priority 8 — Five-target delivery workflows:** before declaring
   ...
   maintained multi-platform patterns in `jm2/CroMagRally` and `jm2/tributary`; make CI and release artifacts cover every
   supported OS/architecture pair, with explicit provenance, smoke, and packaging gates rather than matrix-only claims.
   Utility-only legs do not count: require production jobs for Windows amd64/ARM64, Linux amd64/arm64, and macOS arm64.
   Resolve and validate one exact release tag/SHA, disable persisted checkout credentials, keep build jobs read-only,
   reserve `contents: write` for a final code-free publisher, and publish target-labeled artifacts, a reproducible source
   archive, and aggregate `SHA256SUMS`; add bounded timeouts, superseded-run cancellation, and immutable action pins.

Exact-key audit follow-up status:

- Resolved at `0a87007e`: `TryClassifyStorageIsolation` distinguishes
...
- Open before production enrollment: callback helpers must remain checked and
  ...
  exhaustion is an availability limit, not reusable authority.
